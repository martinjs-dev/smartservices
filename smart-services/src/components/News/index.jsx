...
const News = ({ news_query, onSearchQueryChange }) => {
  const [isEditing, setIsEditing] = useState(!news_query);
  const [news, setNews] = useState(news_query || "");
  ...
  const getNews = async () => {
    ...
    try {
      const response = await axios.get(
        `https://newsapi.org/v2/everything?q=${news}&apiKey=e1ed11560f84472d8a8a69e1a2d67cae`
      );
      const { data } = response;
      if (data.articles?.length == 0) {
        setResult(null);
      } else setResult(data.articles?.slice(0, 20));
      onSearchQueryChange(news);
        ...
        <div className="flex flex-col gap-2 mt-4 h-[500px] overflow-y-auto">
          {result.map((element, index) => (
            <div className="border border-gray-500 shadow mt-4 p-2" key={index}>
              <p>
                <span className="font-bold text-xl text-st">Title:</span>
                {/* {console.log(element)} */}
                {element.title}
              </p>
              <p>
              ...
              <p>
                <span className="font-bold text-xl text-start">Url:</span>{" "}

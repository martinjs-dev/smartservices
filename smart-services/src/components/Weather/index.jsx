...
const Weather = ({ city_name, onSearchQueryChange }) => {
  const [isEditing, setIsEditing] = useState(!city_name); 
  ...
  const getWeather = async () => {
    ...
    try {
      const response = await axios.get(
        `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=4cbefb7220702d23b5225ecc5afcfe21`
      );
      const { data } = response;
      setResult(data);
        ...
        <div className="flex flex-col gap-2 mt-4">
          ...
          <div className="flex justify-between items-center">
            <p>
              ...
              <span className="text-xl">{result?.sys?.country}</span>
          ...
          <div className="flex justify-between items-center">
            <p>
              ...
              <span>{result?.main?.temp}</span>
            </p>
          </div>

          <div className="flex justify-between items-center">
            <p>
              <span className="text-md font-bold">Weather : </span>
              <span>{result?.weather[0]?.description}</span>
            </p>
          </div>
          <div className="flex justify-between items-center">
            <p>
              <span className="text-md font-bold">Low-high : </span>
              <span>Low-high</span>
            </p>
          </div>
          <div className="flex justify-between items-center">
            <p>
              <span className="text-md font-bold">Feels-like : </span>
              <span>{result?.main?.feels_like}</span>
            </p>
          </div>

          <div className="flex justify-between items-center">
            <p>
              <span className="text-md font-bold">Humidity : </span>

...
const Livre = ({ livre_query, onSearchQueryChange }) => {
  const [isEditing, setIsEditing] = useState(!livre_query);
  const [livre, setLivre] = useState(livre_query || "");
        ...
        <div className="flex flex-col gap-2 mt-4 h-[500px] overflow-y-auto">
          {result.map((element, index) => (
            <div className="border border-gray-500 shadow mt-4 p-2" key={index}>
              <p>
                <span className="font-bold text-xl text-st">livre : </span>
                {result?.reading_log_entries[0]?.work?.title}
